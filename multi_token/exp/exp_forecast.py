    ...
    def vali(self, vali_data, vali_loader, criterion, is_test=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                batch_y = batch_y[:, -tgt_len:, :].to(self.device)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                outputs = self.model(batch_x, batch_x_mark, batch_y_mark)
                def sliding_stack(x, window_size=4):
                    """
                    x: tensor of shape (batch, seq_len, feature_dim)
                    e.g. (8192, 30, 96)

                    Returns:
                        out: shape (batch, window_size, num_windows * feature_dim)
                            where num_windows = seq_len - window_size + 1
                            e.g. (8192, 4, 27 * 96) = (8192, 4, 2592)
                    """
                    B, T, F = x.shape
                    assert T >= window_size, "seq_len must be >= window_size"

                    # 1. Sliding windows over seq_len (dim=1)
                    #    Shape: (B, num_windows, window_size, F)
                    windows = x.unfold(dimension=1, size=window_size, step=1)
                    num_windows = windows.size(1)

                    # 2. Move window_size before num_windows: (B, window_size, num_windows, F)
                    windows = windows.permute(0, 2, 1, 3).contiguous()

                    # 3. Flatten (num_windows, F) into one dimension
                    #    Result: (B, window_size, num_windows * F)
                    out = windows.view(B, window_size, num_windows * F)

                    return out
                
                if self.args.dp:
                    torch.cuda.synchronize()
                batch_y = batch_y.reshape(batch_y.size(0), 30, 96)
                batch_y = sliding_stack(batch_y, window_size=4)
                ...
                if global_step % 500 == 0:
                    # only rank 0 writes in ddp
                    if (self.args.ddp and self.args.local_rank == 0) or not self.args.ddp:
                        # handle DataParallel like EarlyStopping does
                        if self.args.dp:
                            model_to_save = self.model.module
                        else:
                            model_to_save = self.model
                        ckpt_path = os.path.join(path, f'checkpoint_step_{global_step}.pth')
